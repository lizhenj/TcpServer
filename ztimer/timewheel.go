...
/*
 tips:
   一个网络服务程序需要管理大量客户端连接的，
   其中每个客户端连接都需要管理它的timeout时间。
   通常连接的超时管理一般设置为30~60s不等，并不需要太精准的时间控制，
   另外由于服务端管理着多达数万到数十万不等的连接数
   因此我们设法为每个连接使用一个Timer,那样太消耗资源

   用时间轮的方式管理和维护大量的timer调度，解决上边问题
*/
...
type TimeWheel struct {
	...
	//互斥锁（继承RWMutex的RWLock,UnLock 等方法）
	sync.RWMutex
}
...
func (tw *TimeWheel) addTimer(tid uint32, t *Timer, forceNext bool) error {
	...
	if delayInterval >= tw.interval {
		...
		//在对应的刻度上的定时器Timer集合map加入当前定时器（由于是环形，所以得求余）
	...
	if delayInterval < tw.interval && tw.nextTimeWheel == nil {
		if forceNext == true {
			...
			//如果手动添加定时器，那么直接将timer添加到对应底层时间轮的当前调度集合中
